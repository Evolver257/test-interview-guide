...
本仓库正在开展软件测试实践作业，目前覆盖简历上传与预处理、面试日程管理两个模块。
详细范围、阶段结果和运行说明见 [`coursework/README.md`](coursework/README.md)。

截至 2026-09-15 的进度：

| 被测模块 | 测试用例 | 当前状态 | 缺陷 |
| -------- | -------- | -------- | ---- |
| 简历上传与预处理 | 30 条 | 30 条回归通过 | 3 项已修复并验证 |
| 面试日程管理 | 10 条 | 测试已提交，数据构造方法待补全 | 1 项已定位并提交修复，待回归验证 |
| 合计 | 40 条 | 30 条已通过，10 条待完成编译验证 | 4 项 |

在仓库根目录执行以下命令可运行全部单元测试并生成 JUnit 与 JaCoCo 报告：

```powershell
.\gradlew.bat :app:test
```

测试使用 Mock 隔离数据库、Redis、对象存储和 AI 服务，不需要配置 API Key。
当前面试日程测试仍缺少 `request(...)` 与 `entity(...)` 两个测试数据构造方法，补全后将执行
全量回归并更新最终统计。此处展示阶段性进度，不代表40条测试已经全部通过。
